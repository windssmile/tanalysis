...
**8 大板块 / 49 条目 / 120 测试全绿 / 11 种自绘图表组件**

| 板块 | id | 条目数 | 内容 |
|---|---|---|---|
| K线基础 | candlestick | 12 | 构成/锤子/十字星/吞没×2/早晨之星/乌云盖顶/刺透/黄昏之星/孕线/流星/上吊 |
...
| 量价关系 | volume | 4 | 量价基础/放量突破/量价背离/筹码分布 |
...
**后续三个工具页（已设计，未实现）** —— 见 `docs/superpowers/specs/2026-06-02-partB-and-tools-design.md`：
- **有效性矩阵**：趋势市/震荡市适用性对照表（独立工具页 `/matrix`）。
- **术语速查 + 搜索**：全站搜索 + curated 术语表（`/search`、`/glossary`）。
- **playground**：参数沙盘 + 信号回测引擎（纯函数引擎 + 可换数据源 + 诚实框架：方法演示标注/随机对照组/多市场情境），为日后接真实数据预留 `DataSource.load()` 接口。
