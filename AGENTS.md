...
**8 大板块 / 49 条目 / 113 测试全绿 / 11 种自绘图表组件**
...
贯穿全站的两条线：**📏 量化刻画**（可度量阈值）+ **📐 量化视角**（统计检验/防过拟合）。风险板块额外有 **⚠ 何时会失效**（`pitfalls`）。
...
**Part B（已规划，未完成）· 把「失效场景」`pitfalls` 铺到现有 45 个条目**
目前 `pitfalls` 只有风险板块的 4 条用了。计划逐板块为其余条目补充结构化"何时会失效"清单（纯内容，渲染已就绪，只需在 topic 的 `sections` 加 `pitfalls` 数组）。建议按板块分批：K线12 → 指标8 → 形态8 → 趋势4+量价4+分时4+实战5。
