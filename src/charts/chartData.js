export const chartData = {
  ...
  // 天量天价·地量地价：前段顶部一根巨量长阴滞涨，后段底部缩量至地量后温和回升
  'volume-climax': {
    candles: fromPath(
      [12, 12.6, 13.2, 13.8, 14.3, 14.6, 13.4, 12.9, 12.4, 12, 11.7, 11.6, 11.65, 11.7, 11.85, 12.1],
      [120, 160, 200, 240, 300, 520, 280, 200, 150, 110, 80, 55, 45, 50, 90, 150],
    ),
    annotations: [
      { type: 'highlight', index: 5 },
      { type: 'highlight', index: 12 },
    ],
  },
  // 换手率：左段低换手缩量、中段高换手放量、右段换手回落
  'turnover-rate': {
    candles: fromPath(
      [10, 10.05, 9.95, 10.1, 10.4, 10.9, 11.5, 12, 12.3, 12.1, 11.9, 11.8, 11.75, 11.7],
      [50, 55, 48, 70, 130, 220, 300, 260, 180, 120, 90, 75, 60, 55],
    ),
    annotations: [
      { type: 'highlight', index: 6 },
  ...
  // 葛兰碧量价法则：量增价升 / 量缩价升 / 量增价跌 / 量缩价跌 四段
  'granville-volume': {
    candles: fromPath(
      [10, 10.4, 10.9, 11.5, 11.7, 11.85, 11.95, 11.6, 11.1, 10.5, 10.3, 10.15, 10.05, 10.0],
      [80, 140, 200, 270, 110, 75, 55, 230, 280, 320, 150, 90, 60, 45],
    ),
    annotations: [],
  },
  // 量价四组合：价涨量增 / 价涨量缩 / 价跌量增 / 价跌量缩 四段
  'volume-price-combos': {
    candles: [
      { o: 10, h: 10.6, l: 9.9, c: 10.5, v: 1200 },
      { o: 10.5, h: 11.2, l: 10.4, c: 11.1, v: 1800 },
      { o: 11.1, h: 11.7, l: 11.0, c: 11.6, v: 2200 },
      { o: 11.6, h: 12.1, l: 11.5, c: 12.0, v: 900 },
      { o: 12.0, h: 12.4, l: 11.9, c: 12.3, v: 700 },
      { o: 12.3, h: 12.5, l: 11.6, c: 11.7, v: 2100 },
      { o: 11.7, h: 11.8, l: 10.9, c: 11.0, v: 2400 },
      { o: 11.0, h: 11.1, l: 10.5, c: 10.6, v: 800 },
      { o: 10.6, h: 10.7, l: 10.2, c: 10.3, v: 650 },
    ],
    annotations: [],
  },

  // 量价时空配合：左段低位温和放量启动、右段高位巨量见顶风险（相同"放量"不同位置）
  'volume-time-space': {
    candles: fromPath(
      [10, 10.1, 10.3, 10.6, 11, 11.6, 12.3, 13, 13.6, 14, 14.2, 14.3],
      [60, 90, 120, 110, 130, 160, 200, 240, 280, 420, 200, 120],
    ),
    annotations: [
      { type: 'highlight', index: 2 },
      { type: 'highlight', index: 9 },
    ],
  },
  // 主力量价手法：吸筹(底部温和放量)→洗盘(缩量回踩)→拉升(放量)→出货(高位放量滞涨)
  'manipulation-volume': {
    candles: fromPath(
      [10, 10.2, 10.4, 10.3, 10.5, 10.2, 10.1, 10.4, 11.2, 12.2, 13.2, 14, 14.5, 14.4, 14.45, 14.3],
      [80, 130, 160, 70, 60, 55, 50, 90, 220, 300, 340, 300, 360, 380, 350, 320],
    ),
    annotations: [
      { type: 'box', from: 0, to: 2, label: '吸筹(叙事)' },
      { type: 'box', from: 3, to: 6, label: '洗盘(叙事)' },
      { type: 'box', from: 7, to: 10, label: '拉升(叙事)' },
      { type: 'box', from: 11, to: 15, label: '出货(叙事)' },
    ],
  },
  // 堆量与峰量：左段连续温和放大的"堆量"、右段单根突放的"峰量"尖峰
  'volume-stacking': {
    candles: fromPath(
      [10, 10.3, 10.6, 11, 11.4, 11.8, 12.1, 12, 12.2, 12.1, 13, 12.6],
      [80, 110, 140, 170, 200, 230, 110, 90, 100, 95, 360, 120],
    ),
    annotations: [
      { type: 'box', from: 0, to: 5, label: '堆量' },
      { type: 'highlight', index: 10 },
    ],
  },
  // 顶底量能特征：前段高位放量滞涨(顶)、后段缩量地量后温和放大(底)
  'volume-top-bottom': {
    candles: fromPath(
      [14, 14.3, 14.5, 14.45, 14.4, 13.8, 13, 12.3, 11.9, 11.7, 11.65, 11.7, 11.9],
      [180, 240, 360, 380, 350, 240, 180, 120, 80, 50, 45, 110, 170],
    ),
    annotations: [
      { type: 'highlight', index: 3 },
      { type: 'highlight', index: 10 },
